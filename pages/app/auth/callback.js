...
// After any successful auth exchange, always land on the restaurants page.
// GPS is fetched live there on every visit.
    ...
    const run = async () => {
      try {
        ...
        if (code) {
          ...
          window.location.replace(DELIVERY_REDIRECT);
          return;
        }
        ...
        // No code or tokens found — likely a direct visit
        setTimeout(() => setErr("Invalid Magic Link"), 1500);
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-6">
      <AnimatePresence mode="wait">
        {err ? (
          ...
          >
            <motion.div
              className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 relative"
              animate={{ x: [-5, 5, -5, 5, 0] }}
            ...
            <h1 className="text-2xl font-extrabold text-[#111827] mb-2">Link Expired</h1>
          ...
          >
            ...
            <h2 className="text-xl font-bold text-gray-800 animate-pulse">Signing you in...</h2>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
