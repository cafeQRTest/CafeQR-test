...
const handleEditSave = async (edited) => {
 ...
 setEditingOrder(null);
    // ✅ Use order_for_print for KOT (matches create API exactly)
    window.dispatchEvent(
      new CustomEvent('auto-print-order', {
        detail: {
          ...data.order_for_print,  // ✅ Full KOT data structure
          autoPrint: true,
          kind: 'kot',
          isEdited: true,
          // ✅ Bonus: pass changed_items for delta printing if needed
          changed_items: data.order_for_print.changed_items || [],
        },
      })
    );
...
useEffect(() => {
 ...
 const finalize = async (order) => {
  ...
  // Credit orders don't need payment dialog
  ...
  if (invoice) {
    const total = Number(invoice.total_inc_tax ?? 0);
    const paid = Number(invoice.paid_amount ?? 0);
    const diff = paid - total;

    // Fully paid (within 1 paise tolerance) => auto-complete
    if (Math.abs(diff) <= 0.01) {
      complete(order.id);
      return;
    }

    // Underpaid: need to collect remaining
    if (diff < -0.01) {
      const remaining = Math.max(0, total - paid);
      setPaymentConfirmDialog({
        ...order,
        invoice,
        mode: 'collect',          // custom flag: collecting remaining
    ...
    // Overpaid: need to refund
    if (diff > 0.01) {
      const refund = diff;
      setPaymentConfirmDialog({
        ...order,
        invoice,
        mode: 'refund',           // custom flag: refunding extra
        refundAmount: refund,
        alreadyPaidAmount: paid,
        totalAmount: total,
      });
  ...
  // No invoice = normal counter payment dialog
