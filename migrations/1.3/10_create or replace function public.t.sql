...
begin
  ...
  if v_phone is not null then
    ...
    if v_customer_id is null then
      -- If phone is new, try unique name match within this restaurant (optional merge)
      if v_name is not null then
        ...
        if v_name_match_count = 1 then
          ...
          -- Only set phone if that customer doesn't already have one
      ...
      -- Improve name if missing on master
      if v_name is not null then
        update public.customers
    ...
    return new; -- no identifying info, skip
  end if;
  ...
  -- 3) Upsert restaurant_customers + visits
  ...
  on conflict (restaurant_id, customer_id) do update set
    first_order_at = least(public.restaurant_customers.first_order_at, excluded.first_order_at),
    last_order_at  = greatest(public.restaurant_customers.last_order_at, excluded.last_order_at),
    ...
    visit_count    = case
                      when public.restaurant_customers.last_visit_date is distinct from excluded.last_visit_date
                      then public.restaurant_customers.visit_count + 1
                      else public.restaurant_customers.visit_count
                     end,
    last_visit_date = greatest(coalesce(public.restaurant_customers.last_visit_date, excluded.last_visit_date), excluded.last_visit_date),
    updated_at = now();
