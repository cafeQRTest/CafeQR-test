...
export function useRequireAuth() {
  ...
  useEffect(() => {
    ...
    async function run() {
      // Cold start bootstrap (restoration is handled by client adapter)
      ...
      if (PUBLIC_PATHS.includes(path)) {
        if (session) {
          const redirect = router.query?.redirect ? String(router.query.redirect) : '/owner/counter';
          setChecking(false);
