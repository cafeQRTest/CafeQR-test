...
channel = supabase
  ...
  .on(
    ...
    { event: 'order-edited' },
    (payload) => {
      console.log('[DEBUG_PRINT] Received broadcast:', payload); // DEBUG LOG
      if (!alive) return;
      const data = payload.payload; // Payload is wrapped in { event, payload, type }
      
      console.log('[DEBUG_PRINT] Broadcast payload data:', data); // DEBUG LOG

      if (!data || !data.id) return;
      
      // Prevent duplicate if this device originated the edit (handled by local dispatch)
      // OR if we already processed it.
      if (hasPrintedRecently(data.id, 'kot')) {
          console.log('[DEBUG_PRINT] print dedup skipped', data.id); // DEBUG LOG
          return;
      }

      console.log('[DEBUG_PRINT] Processing print...', data.id); // DEBUG LOG
      markPrinted(data.id, 'kot'); // Mark as printed so we don't double print
  ...
  .subscribe((status) => console.log('[DEBUG_PRINT] Channel Status:', status)); // DEBUG LOG
