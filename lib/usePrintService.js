...
channel = supabase
  ...
  .on(
    ...
    { event: 'order-edited' },
    (payload) => {
      console.log('🔔 [usePrintService] Broadcast received:', payload);
      
      if (!alive) {
        console.log('❌ [usePrintService] Not alive, ignoring');
        return;
      }
      
      const data = payload.payload;
      console.log('📦 [usePrintService] Payload data:', data);
      
      if (!data || !data.id) {
        console.log('❌ [usePrintService] No data or ID, ignoring');
        return;
      }
      
      // Prevent duplicate if we already printed this edit
      if (hasPrintedRecently(data.id, 'kot')) {
        console.log('⏭️ [usePrintService] Already printed recently, skipping:', data.id);
        return;
      }

      console.log('✅ [usePrintService] Dispatching auto-print-order event for:', data.id);
      ...
      console.log('✨ [usePrintService] Event dispatched successfully');
    }
  )
  .subscribe();
