...
channel = supabase
  ...
  .on(
    'postgres_changes',
    {
      event: 'INSERT',
      schema: 'public',
      table: 'kot_print_queue',
      filter: `restaurant_id=eq.${restaurant.id}`,
    },
    async (payload) => {
      if (!alive) return;
      
      const queueId = payload?.new?.id;
      const printData = payload?.new?.print_data;
      const orderId = payload?.new?.order_id;
      
      console.log('[PRINT SERVICE] KOT print queue INSERT detected:', orderId);
      
      if (!printData || !orderId) {
        console.log('[PRINT SERVICE] Invalid print data, ignoring');
        return;
      }
      
      // Check if we already printed this recently
      if (hasPrintedRecently(orderId, 'kot')) {
        console.log('[PRINT SERVICE] Already printed recently, marking as processed');
        // Mark as processed even if we skip printing
        await supabase
      ...
      console.log('[PRINT SERVICE] Dispatching KOT print for edited order');
      
      // DO NOT markPrinted here! KotPrint component handles that.
      // If we mark it here, KotPrint will think it's a duplicate and skip it.
      ...
      console.log('[PRINT SERVICE] KOT print dispatched for order:', orderId);
    }
  )
  .subscribe();
