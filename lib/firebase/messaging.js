...
async function ensureMessagingServiceWorker() {
  ...
  try {
    ...
    if (isDev) console.warn('[push:web] service worker registration failed:', e?.message || e);
    return null;
  }
}
...
function attachForegroundNotificationHandler(messaging) {
  ...
  onMessage(messaging, async (payload) => {
    try {
      ...
      if (isDev) console.warn('[push:web] foreground notification failed:', e?.message || e);
    }
  });
}
...
export async function getFCMToken({ requestPermission = false } = {}) {
  ...
  try {
    ...
    if (!registration) {
      if (isDev) console.warn('[push:web] No service worker registration available for FCM');
      return null;
    }
    ...
    if (isDev) console.error('[push:web] getToken failed:', e?.message || e);
    return null;
  }
}
