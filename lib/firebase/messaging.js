...
const FCM_SW_VERSION = '20260305-2';
...
function attachForegroundNotificationHandler(messaging) {
  ...
  onMessage(messaging, async (payload) => {
    ...
    // Payload verification to prevent cross-restaurant bleed
    const targetRestaurantId = payload?.data?.restaurantId || payload?.data?.rid;
    const activeRestaurantId = typeof window !== 'undefined' ? localStorage.getItem('active_restaurant_id') : null;

    if (targetRestaurantId && activeRestaurantId && targetRestaurantId !== activeRestaurantId) {
      console.warn(`[push:web] Blocked foreground notification for mismatched restaurant (Target: ${targetRestaurantId}, Active: ${activeRestaurantId})`);
      return;
    }
...
export async function getFCMToken({ requestPermission = false } = {}) {
  ...
  try {
    // Always refresh the worker registration first so stale SW code gets replaced
    // even before notification permission is granted.
