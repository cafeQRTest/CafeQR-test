...
export async function generateBillPdf(invoice, items, totals, restaurant = {}) {
  ...
  if (rates.length > 0) {
     rates.forEach(rate => {
        ...
        drawRow(`CGST @ ${halfRate}%`, `+ ₹${halfAmount.toFixed(2)}`);
        drawRow(`SGST @ ${halfRate}%`, `+ ₹${halfAmount.toFixed(2)}`);
     });
  } else if (Number(totals.total_tax) > 0) {
      // Fallback if item-level tax missing but header has total
      const halfTax = totals.total_tax / 2;
      drawRow(`CGST Check`, `+ ₹${halfTax.toFixed(2)}`);
      drawRow(`SGST Check`, `+ ₹${halfTax.toFixed(2)}`);
  }
