...
export async function generateBillPdf(invoice, items, totals, restaurant = {}) {
  ...
  const wTotal = 70;
  const wTaxAmt= 50; // was wTax
  const wTaxRate= 30; // NEW
  const wDisc  = 40; // Reduced from 50
  const wRate  = 50; // Reduced from 60
  const wQty   = 30; // Reduced from 40
  
  // Right-to-Left calculation
  const xTotal   = xRight - wTotal;
  const xTaxAmt  = xTotal - wTaxAmt;
  const xTaxRate = xTaxAmt - wTaxRate; // New Col Start
  const xDisc    = hasLineDisc ? (xTaxRate - wDisc) : xTaxRate;
  ...
  doc.text('TAX%', xTaxRate, headerY, { width: wTaxRate, align: 'right' });
  doc.text('GST', xTaxAmt, headerY, { width: wTaxAmt, align: 'right' });
  ...
  items.forEach((item) => {
    ...
    doc.text(taxRate > 0 ? `${taxRate}%` : '-', xTaxRate, y, { width: wTaxRate, align: 'right' });
    doc.text(taxAmt > 0 ? taxAmt.toFixed(2) : '-', xTaxAmt, y, { width: wTaxAmt, align: 'right' });
  ...
  // --- DYNAMIC TAX BUCKET DISPLAY ---
  // Aggregate taxes by rate from items list
  const taxBuckets = {}; // { "5": 10.00, "28": 50.00 }
  
  items.forEach(item => {
      const r = Number(item.tax_rate || 0);
      const t = Number(item.tax_amount || 0);
      if (r > 0 && t > 0) {
          taxBuckets[r] = (taxBuckets[r] || 0) + t;
      }
  });

  // Sort rates ascending
  const rates = Object.keys(taxBuckets).map(Number).sort((a,b) => a - b);
  
  if (rates.length > 0) {
     rates.forEach(rate => {
        const amount = taxBuckets[rate];
        const halfRate = rate / 2;
        const c = Math.round((amount / 2) * 100) / 100;
        const s = Math.round((amount / 2) * 100) / 100;
        
        // Label format: "CGST (2.5%)" / "SGST (2.5%)"
        drawRow(`CGST @ ${halfRate}%`, `+ ₹${c.toFixed(2)}`);
        drawRow(`SGST @ ${halfRate}%`, `+ ₹${s.toFixed(2)}`);
     });
  } else if (Number(totals.total_tax) > 0) {
      // Fallback if item-level tax missing but header has total
      const totalTaxVal = Number(totals.total_tax);
      const c = Math.round((totalTaxVal / 2) * 100) / 100;
      const s = Math.round((totalTaxVal / 2) * 100) / 100;
      drawRow(`CGST Check`, `+ ₹${c.toFixed(2)}`);
      drawRow(`SGST Check`, `+ ₹${s.toFixed(2)}`);
  }
  ...
  // Grand Total Pill
  footerY += 8; // Bit more breathing room before the big total
