// lib/generateBillPdf.js - CORRECTED
...
export async function generateBillPdf(payload, restaurantId) {
  ...
  // ✅ FIX: Declare payMethod and mixed EARLY - before using in template literals
  ...
  // ✅ FIX: Build paymentDisplay now that payMethod is defined
  let paymentDisplay = payMethod
  if (payMethod === 'MIXED' && mixed) {
    paymentDisplay = `MIXED (₹${mixed.cash_amount} Cash + ₹${mixed.online_amount} ${mixed.online_method?.toUpperCase() || 'Online'})`
  }
  ...
  doc.text(`Date: ${istDateString}`, rightX + 12, topY + 40)
  doc.text(`Payment: ${paymentDisplay}`, rightX + 12, topY + 54)
  ...
  const pathKey = `bills/${restaurantId}/${safeName}.pdf`
