...
export async function generateBillPdf(payload, restaurantId) {
  ...
  const items = (itemsIn.length ? itemsIn : [
    ...
    uom_precision: it.uom_precision,
    uom_short_code: it.uom_short_code,
  }))
  ...
  const invRoundOff = Number(invoice.round_off_amount || 0)
  ...
  // Show Daily Bill Number if available
  // Customer Details Block
  if (invoice.customer_name) {
    const custX = leftX
    const custY = Math.max(doc.y + 12, topY + metaH + 8)
    
    doc.fillColor(dark).font('Bold').fontSize(10)
    doc.text('Invoice To:', custX, custY)
    doc.font('Regular').fontSize(10).fillColor(dark)
    doc.text(invoice.customer_name, custX, custY + 14)

    if (invoice.customer_gstin) {
      doc.fontSize(9).fillColor(gray).text(`GSTIN: ${invoice.customer_gstin}`, custX, doc.y + 2)
    }

    // Since we printed customer info, push the table down a bit
    // But we need to communicate this new Y position to the rest of the layout logic
    // The current layout logic uses `headerBottom`. We can just ensure we write `headerBottom` logic after this?
    // Wait, the current code draws a line at `headerBottom`. Let's assume headerBottom is adaptable or defined below.
  }
  ...
  // Column widths with Discount column added
  const hasDiscounts = items.some(it => (it.discount_amount || 0) > 0)
  const col = hasDiscounts ? {
    hsn:  { x: leftX, w: 45 },
    desc: { x: leftX + 49, w: contentWidth - (49 + 35 + 60 + 50 + 35 + 60 + 75) },
    qty:  { x: pageWidth - 40 - (35 + 60 + 50 + 35 + 60 + 75), w: 35 },
    rate: { x: pageWidth - 40 - (60 + 50 + 35 + 60 + 75), w: 60 },
    disc: { x: pageWidth - 40 - (50 + 35 + 60 + 75), w: 50 },
    taxp: { x: pageWidth - 40 - (35 + 60 + 75), w: 35 },
    taxa: { x: pageWidth - 40 - (60 + 75), w: 60 },
    total:{ x: pageWidth - 40 - 75, w: 75 },
  } : {
    hsn:  { x: leftX, w: 50 },
    desc: { x: leftX + 54, w: contentWidth - (54 + 40 + 70 + 40 + 70 + 85) },
    qty:  { x: pageWidth - 40 - (40 + 70 + 40 + 70 + 85), w: 40 },
    rate: { x: pageWidth - 40 - (70 + 40 + 70 + 85), w: 70 },
    taxp: { x: pageWidth - 40 - (40 + 70 + 85), w: 40 },
    taxa: { x: pageWidth - 40 - (70 + 85), w: 70 },
    total:{ x: pageWidth - 40 - 85, w: 85 },
  }
  ...
  if (hasDiscounts) {
    doc.text('Disc (₹)', col.disc.x, tableTop, { width: col.disc.w, align: 'right' })
  }
  ...
  /* fmtQty now uses per-item precision */
  const fmtQty = (q, precision = 2) => {
    const n = Number(q ?? 0)
    return Number.isFinite(n) ? n.toFixed(precision) : '0.00'
  }

  items.forEach((it) => {
    // Calculate dynamic height for description
    const descHeight = doc.heightOfString(it.name, { width: col.desc.w })
    const rowH = Math.max(descHeight, 16)
    ...
    // Use standard doc.text to allow wrapping
    doc.text(it.name, col.desc.x, y, { width: col.desc.w })
    
    // Use item-specific precision for quantity (fallback to 2 if missing, though 0 is valid)
    const p = it.uom_precision ?? 2
    let qtyStr = fmtQty(it.qty, p)
    if (it.uom_short_code) {
       qtyStr += ` ${it.uom_short_code}`
    }
    
    // Calculate original unit price (before discount)
    const originalUnitPrice = (unitAmt * it.qty + it.discount_amount) / it.qty
    
    doc.text(qtyStr, col.qty.x, y, { width: col.qty.w, align: 'right' })
    doc.text(money(originalUnitPrice), col.rate.x, y, { width: col.rate.w, align: 'right' })
    if (hasDiscounts) {
      const discText = it.discount_amount > 0 ? `-${money(it.discount_amount)}` : '-'
      doc.text(discText, col.disc.x, y, { width: col.disc.w, align: 'right' })
    }
    ...
    doc.moveTo(leftX, y + 2).lineTo(pageWidth - 40, y + 2)
      ...
      .lineWidth(0.5)
      .stroke()
    y += 4 // increased padding slightly from 2 to 4 for better breathing room
  })

  // Totals
  const totalsX = pageWidth - 40 - 260
  const totalsY = Math.max(y + 10, bottomLimit - 110)
  const totalsW = 260
  let totalsRows = 3; // Subtotal, Tax, Grand Total
  if (invDiscount > 0) totalsRows++;
  if (invRoundOff !== 0) totalsRows++;
  const totalsH = 20 + (totalsRows * 20); // Dynamic height based on rows
  ...
  twoCol(doc, 'Subtotal', money(invSubtotal), totalsX + 12, totalsX + 12, currentY, totalsW - 24)
  
  currentY += 20
  twoCol(doc, 'Total Tax', money(invTax), totalsX + 12, totalsX + 12, currentY, totalsW - 24)
  
  currentY += 24
  if (invDiscount > 0) {
    twoCol(doc, 'Discount', '-' + money(invDiscount), totalsX + 12, totalsX + 12, currentY, totalsW - 24)
    currentY += 24
  }

  if (invRoundOff !== 0) {
    twoCol(doc, 'Round off', (invRoundOff > 0 ? '+' : '') + money(invRoundOff), totalsX + 12, totalsX + 12, currentY, totalsW - 24)
    currentY += 24
  }

  doc.font('Bold')
  twoCol(doc, 'Grand Total', money(invTotal), totalsX + 12, totalsX + 12, currentY, totalsW - 24)
  doc.font('Regular')
